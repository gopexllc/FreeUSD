...
bool parse_typed_value_payload(UsdcCrateTypedValueKind kind, const std::vector<std::uint8_t>& bytes,
  ...
  switch (kind) {
    ...
    case UsdcCrateTypedValueKind::Vec4f:
      if (bytes.size() != 16u) {
        set_detail(err_out, "USDC typed Vec4f payload size mismatch");
        return false;
      }
      std::memcpy(out->vec4f_value.data, bytes.data(), 16u);
      return true;
    case UsdcCrateTypedValueKind::Vec2f:
      if (bytes.size() != 8u) {
        set_detail(err_out, "USDC typed Vec2f payload size mismatch");
        return false;
      }
    ...
    case UsdcCrateTypedValueKind::Vec2d:
      ...
      return true;
    case UsdcCrateTypedValueKind::Quatf:
      if (bytes.size() != 16u) {
        set_detail(err_out, "USDC typed Quatf payload size mismatch");
        return false;
      }
      std::memcpy(&out->quatf_value.real, bytes.data(), 16u);
      return true;
    case UsdcCrateTypedValueKind::Quatd:
      if (bytes.size() != 32u) {
        set_detail(err_out, "USDC typed Quatd payload size mismatch");
        return false;
      }
      std::memcpy(&out->quatd_value.real, bytes.data(), 32u);
      return true;
...
bool ReadUsdCrateTypedValuesTableFromPath(const std::string& path, UsdcCrateTypedValuesTable& out,
  ...
  for (std::uint64_t i = 0; i < count; ++i) {
    ...
    cursor += static_cast<std::size_t>(len);
    if (kind_raw > static_cast<std::uint64_t>(UsdcCrateTypedValueKind::Quatd)) {
