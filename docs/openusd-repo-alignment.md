...
- **`freeusd.usd.crate`** — **`usdc_crate_identifier()`**, **`detect_usd_file_kind_from_path`**, **`read_usdc_bootstrap_from_path`**, **`read_usdc_toc_from_path`**, **`UsdFileKind`**: magic + path sniff + **bootstrap** + **TOC section list** in Python/C++; **no** full `.usdc` payload decode; the C ABI and Go / Rust expose matching entry points for bootstrap and TOC.
