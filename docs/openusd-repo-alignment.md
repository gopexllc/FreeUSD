...
| `usd/usd/crateFile` (binary crate) | `include/freeusd/usd/crateFile.hpp` | **`UsdcCrateIdentifier`**, **`DetectUsdFileKindFromPath`**, **`ReadUsdCrateBootstrapFromPath`**, **`ReadUsdCrateTocFromPath`**, **`ReadUsdCrateSectionBytesFromPath`**, validated **`TOKENS`** / **`STRINGS`** / **`PATHS`** / **`FIELDS`** table reads, and a narrow embedded-`USDA` section bridge |
...
- **`freeusd.usd.crate`**: **`usdc_crate_identifier()`**, **`detect_usd_file_kind_from_path`**, **`read_usdc_bootstrap_from_path`**, **`read_usdc_toc_from_path`**, **`read_usdc_section_bytes_from_path`**, **`read_usdc_token_table_from_path`**, **`read_usdc_string_table_from_path`**, **`read_usdc_path_table_from_path`**, **`read_usdc_fields_table_from_path`**, **`UsdFileKind`**: magic + path sniff + **bootstrap** + **TOC section list** + raw section payload bytes + validated table reads in Python/C++; full `.usdc` payload decode is still future work, and the C ABI plus Go / Rust expose the same validated low-level slice.
...
- **`freeusd.usd.Stage`** adds **`read_field_at_time`** (composed **`vt.Value`** or **`None`**) plus typed **`read_field_double`**, **`read_field_float`**, **`read_field_bool`**, **`read_field_int64`**, **`read_field_string`**, **`read_field_vec3d`**, **`read_field_vec3f`**, **`read_field_matrix4d`**, **`read_field_quatd`**, **`read_field_quatf`**, **`read_field_token`**, **`read_field_token_array`**. Python follows the same conversion rules as the C ABI: **`read_field_string`** accepts **`string`** or **`token`** payloads, **`read_field_vec3f`** narrows **`Vec3d`**, and **`read_field_quatf`** narrows **`quatd`**; missing prims / missing attributes / wrong-type reads return **`None`**. **`read_field_at_time`** and the same **`read_field_*`** names exist on **`freeusd.usd.Prim`** with **`(name, time=…)`** (no path argument; invalid prims or destroyed stages yield **`None`**).
