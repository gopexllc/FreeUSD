...
| `sdf` (scene description) | `freeusd::sdf` | `Path`, `Layer`, `FieldOpinion`, prim **`customData`**, **attribute connections** (`SetAttributeConnection` / USDA `*.connect` → property path target), prim **references** (`PrimReference`: asset + optional prim path payload), prim **relationships** (`Set`/`Prepend`/`Append`/`DeleteRelationshipTargets`) |
