...
- `partial`: Native D (`native/d`) is a separate clean-room implementation track with an initial USDA reader and in-memory stage for def hierarchy, basic authored values, and simple composed reads through `inherits`, validated against `usd_cross_language.usda` and `parity_imageable.usda`; it does not yet implement sublayers, references, payloads, variants, schemas, or binary crate decode.
