...
- `partial`: Native D (`native/d`) is a separate clean-room implementation track with an initial USDA reader and in-memory stage for def hierarchy, basic authored values, simple strongest-to-weakest sublayer reads, selected variants, and simple composed reads through `inherits`, default-prim `references`, and default-prim `payloads`, validated against `usd_cross_language.usda`, `parity_imageable.usda`, `parity_stack_root.usda`, `parity_variants.usda`, `parity_variant_selection_refs.usda`, `parity_physics_rigid_body_refs.usda`, and `parity_namespace.usda`; it does not yet implement sublayer offsets, schemas, or binary crate decode.
