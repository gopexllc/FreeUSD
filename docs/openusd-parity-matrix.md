...
- `partial`: Native D (`native/d`) is a separate clean-room implementation track with an initial USDA reader and in-memory stage for def hierarchy plus basic authored values, validated against `usd_cross_language.usda` and `parity_imageable.usda`; it does not yet implement composition arcs, sublayers, references, variants, schemas, or binary crate decode.
