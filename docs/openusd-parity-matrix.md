...
- `partial`: Native D (`native/d`) is a separate clean-room implementation track with an initial USDA reader and in-memory stage for def hierarchy, basic authored values, and simple composed reads through `inherits` plus default-prim `references`, validated against `usd_cross_language.usda`, `parity_imageable.usda`, `parity_physics_rigid_body_refs.usda`, and `parity_namespace.usda`; it does not yet implement sublayers, payloads, variants, schemas, or binary crate decode.
