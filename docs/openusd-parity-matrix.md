...
- `partial`: Python remains the broadest wrapper; the C ABI, Go, and Rust now follow the validated crate bootstrap/TOC/raw-section/table subset, composed prim `kind` / `active`, `usdGeom` imageable/boundable, `usdShade` preview-surface diffuse/texture helpers, and `usdSkel` joint/skinning helpers, but broader runtime/schema helpers are still not fully mirrored outside Python/C++.
