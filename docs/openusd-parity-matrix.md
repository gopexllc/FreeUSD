...
- `partial`: Python remains the broadest wrapper; the C ABI, Go, and Rust now follow the validated crate bootstrap/TOC/raw-section/table subset, composed prim `kind` / `active`, `usdGeom` imageable/boundable, `usdShade` PreviewSurface color / scalar / PBR texture helpers, `usdLux` DistantLight sampling, `usdVol` OpenVDBAsset reads, `usdPhysics` PhysicsScene gravity, RigidBodyAPI, CollisionAPI, MassAPI, and FixedJoint reads, and `usdSkel` joint/skinning helpers. D now has an initial C ABI-backed smoke slice for stage open, prim validity, composed prim `kind` / `active`, double/string field reads, `usdGeom` transform / visibility / purpose / bounds helpers, `usdShade` PreviewSurface color / scalar / PBR texture helpers, `usdLux` DistantLight sampling, `usdVol` OpenVDBAsset reads, and selected `usdPhysics` sample reads. Broader runtime/schema helpers are still not fully mirrored outside Python/C++.
