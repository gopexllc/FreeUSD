...
- `partial`: Python remains the broadest wrapper; the C ABI, Go, and Rust now follow the validated crate bootstrap/TOC/raw-section/table subset, composed prim `kind` / `active`, `usdGeom` imageable/boundable, `usdShade` preview-surface diffuse/texture helpers, `usdLux` light-family sampling (`DistantLight`, `SphereLight`, `RectLight`, `DiskLight`, `CylinderLight`, and `DomeLight`), `usdVol` OpenVDBAsset reads, `usdPhysics` PhysicsScene gravity, RigidBodyAPI, CollisionAPI, MassAPI, and FixedJoint reads, and `usdSkel` joint/skinning helpers. D now has an initial C ABI-backed smoke slice for stage open, prim validity, composed prim `kind` / `active`, double/string field reads, `usdGeom` transform / visibility / purpose / bounds helpers, `usdShade` preview-surface helpers, `usdLux` light-family sampling, `usdVol` OpenVDBAsset reads, and selected `usdPhysics` sample reads. Broader runtime/schema helpers are still not fully mirrored outside Python/C++.
- `partial`: Native D (`native/d`) is a separate clean-room implementation track with an initial USDA reader and in-memory stage for root layer metadata, def hierarchy, prim specifier / `kind` / `active` metadata, basic authored values (including `asset`, `matrix4d`, `quatd`, and `quatf`), field and relationship name inspection, prim `customData`, exact `timeSamples`, relationship targets, simple transform / visibility / purpose / bounds helpers, simple `usdVol` and `usdPhysics` schema helpers, simple relocates and prefix substitutions, simple strongest-to-weakest sublayer reads with authored offsets, selected variants, and simple composed reads through `inherits`, `specializes`, default-prim `references`, and default-prim `payloads`, validated against `usd_cross_language.usda`, `parity_imageable.usda`, `parity_stack_root.usda`, `parity_specializes.usda`, `parity_kind_active_refs.usda`, `parity_kind_active_specializes.usda`, `parity_custom_data_inherit.usda`, `parity_custom_data_refs.usda`, `parity_custom_data_specializes.usda`, `parity_variants.usda`, `parity_variant_selection_refs.usda`, `parity_physics_scene.usda`, `parity_physics_rigid_body_kinematic.usda`, `parity_physics_collision.usda`, `parity_physics_mass.usda`, `parity_physics_fixed_joint.usda`, `parity_vol_openvdb.usda`, `parity_vol_volume.usda`, `parity_physics_rigid_body_refs.usda`, and `parity_namespace.usda`; broader schemas and binary crate decode remain incomplete.
