...
- `partial`: USDC bootstrap parsing, TOC parsing, raw section-payload reads, validated `TOKENS` / `STRINGS` / `PATHS` / `FIELDS` / `FIELDSETS` / `SPECS` / `VALUES` table decode on `parity_tables.usdc` (fixture typed kinds: Int32=42, Float=1.5, TokenIndex=0, Bool=true, Double=3.25, Int64=-9007199254740991, StringUtf8=`parity`, plus opaque section payloads), and a narrow embedded-`USDA` stage-open fallback are available in C++; the C ABI follows the same validated open/query slice (`freeusd_read_usdc_typed_values_table_from_path_utf8`).
