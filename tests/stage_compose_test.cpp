...
int main() {
  ...
  strong->SetField(src, Token("moved"), Value::MakeDouble(7.0));
  strong->SetField(Path::FromString("/RelocateSrc/Child"), Token("moved"), Value::MakeDouble(8.0));
  ...
  assert(!composed->PrimPathInUse(src));
  assert(composed->PrimPathInUse(dst_strong));
  assert(composed->PrimPathInUse(Path::FromString("/RelocateStrong/Child")));
  Value moved;
  assert(composed->ReadFieldAtEvaluatedTime(dst_strong, Token("moved"), 1.0, &moved));
  assert(moved.GetDouble(&d));
  assert(d == 7.0);
  moved = composed->GetPrimAtPath(Path::FromString("/RelocateStrong/Child")).GetAttribute(Token("moved"));
  assert(moved.GetDouble(&d));
  assert(d == 8.0);
  ...
  const Path pref_arc = Path::FromString("/PrefArc");
  strong->AddPrimReference(pref_arc, freeusd::sdf::PrimReference{"/Models/tree.usda", {}});
  strong->AddPrimPayload(pref_arc, freeusd::sdf::PrimReference{"/Models/tree_payload.usdc", {}});
  const auto pref_refs = composed->ReadPrimReferences(pref_arc);
  assert(pref_refs.size() == 1u);
  assert(pref_refs[0].asset_path == "/ModelsV2/tree.usda");
  const auto pref_payloads = composed->ReadPrimPayloads(pref_arc);
  assert(pref_payloads.size() == 1u);
  assert(pref_payloads[0].asset_path == "/ModelsV2/tree_payload.usdc");
