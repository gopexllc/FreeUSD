...
def test_parity_tables_fixture_decodes_structured_usdc_tables() -> None:
    ...
    ok, typed, err = read_usdc_typed_values_table_from_path(usdc, 17, 1024)
    assert ok and err == ""
    ...
    assert typed[13]["kind"] == 14 and typed[13]["vec2f_value"] == (0.5, 1.25)
    assert typed[14]["kind"] == 15 and typed[14]["vec4f_value"] == (1.0, 2.0, 3.0, 4.0)
    assert typed[15]["kind"] == 16 and typed[15]["vec2d_value"] == (0.5, 1.75)

    ok, values, err = read_usdc_values_table_from_path(usdc, 17, 1024)
    assert ok and err == ""
