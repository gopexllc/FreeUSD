...
int main() {
...

  {
    ...
    assert(freeusd::usd::crate::ReadUsdCrateTypedValuesTableFromPath(fixture("parity_tables.usdc"), typed_values, 18,
                                                                     1024, &err));
    assert(typed_values.entries.size() == 17u);
    ...
    assert(freeusd::usd::crate::ReadUsdCrateValuesTableFromPath(fixture("parity_tables.usdc"), values, 18, 1024, &err));
    assert(values.entries.size() == 17u);
    assert(values.entries[0].bytes.size() == 4u);
    assert(freeusd::usd::crate::ReadUsdCrateTypedValuesTableFromPath(
        fixture("parity_tables_zlib.usdc"), typed_values, 18, 1024, &err));
    assert(typed_values.entries.size() == 17u);
    assert(freeusd::usd::crate::ReadUsdCrateTypedValuesTableFromPath(
        fixture("parity_tables_lz4.usdc"), typed_values, 18, 1024, &err));
    assert(typed_values.entries.size() == 17u);
    assert(typed_values.entries[11].float_array.size() == 2u);
